...
public class BeanDefinitionParserDelegate {
	...
	public BeanDefinitionHolder parseBeanDefinitionElement(Element ele, @Nullable BeanDefinition containingBean) {
		...
		// 例如：<bean class="com.mgw.Dog" name="myDog1,myDog2"/>，使用myDog1作为beanName
		String beanName = id;
		...
		// 例如：<bean class="com.mgw.Dog"/>，<bean factory-bean="dog4"/>，<bean parent="outer"/>等
	...
	public AbstractBeanDefinition parseBeanDefinitionAttributes(Element ele, String beanName,
		...
		if (ele.hasAttribute(SINGLETON_ATTRIBUTE)) {
			...
			/**
			 <bean id="outer" class="com.mgw.Outer" scope="prototype">
				 <property name="inner">
				 	<bean id="inner" class="com.mgw.Inner"/>
				 </property>
			 </bean>
			 **/
			// Take default from containing bean in case of an inner bean definition.
	...
	public BeanDefinition parseCustomElement(Element ele, @Nullable BeanDefinition containingBd) {
		...
		/*
		* 使用META-INF/spring.handlers文件 根据配置去解析相关的标签处理器
		* 规则为:
		* 根据前缀命名空间http://www.springframework.org/schema/context为key值,然后再去META-INF/spring.handlers查找对应的handler
		* 然后又再使用handler去解析标签或配置类
		*
		* 部分.handlers文件内容展示:
		...
		* */
		...
		// 3、使用命名空间解析器解析自定义标签 委托给NamespaceHandlerSupport.parse()方法
