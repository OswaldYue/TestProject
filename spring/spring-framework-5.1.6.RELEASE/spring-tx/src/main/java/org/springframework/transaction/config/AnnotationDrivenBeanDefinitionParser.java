...
class AnnotationDrivenBeanDefinitionParser implements BeanDefinitionParser {
	...
	public BeanDefinition parse(Element element, ParserContext parserContext) {
		...
		if ("aspectj".equals(mode)) {
			...
			// proxy模式
			// mode="proxy"  AopAutoProxyConfigurer是其内部类
	...
	private static class AopAutoProxyConfigurer {
		...
		public static void configureAutoProxyCreator(Element element, ParserContext parserContext) {
			...
			if (!parserContext.getRegistry().containsBeanDefinition(txAdvisorBeanName)) {
				...
				//设置事务管理器 将事务管理器注入进事务拦截器中
				registerTransactionManager(element, interceptorDef);
				//将事务属性管理属性源注入到事务拦截器中 就是上面的AnnotationTransactionAttributeSource
				interceptorDef.getPropertyValues().add("transactionAttributeSource", new RuntimeBeanReference(sourceName));
				String interceptorName = parserContext.getReaderContext().registerWithGeneratedName(interceptorDef);

				// 4、创建BeanFactoryTransactionAttributeSourceAdvisor的BeanDefinition
				//注册一个事务增强器
				// BeanFactoryTransactionAttributeSourceAdvisor为Advisor
				// TransactionInterceptor相当于Advice
				// AnnotationTransactionAttributeSource相当于PointCut

				// Create the TransactionAttributeSourceAdvisor definition.
				RootBeanDefinition advisorDef = new RootBeanDefinition(BeanFactoryTransactionAttributeSourceAdvisor.class);
				advisorDef.setSource(eleSource);
				advisorDef.setRole(BeanDefinition.ROLE_INFRASTRUCTURE);
				//将事务属性管理设置到事务增强器中 就是上面的AnnotationTransactionAttributeSource
				advisorDef.getPropertyValues().add("transactionAttributeSource", new RuntimeBeanReference(sourceName));
				//将事务拦截器设置到事务增强器中 就是上面的TransactionInterceptor
